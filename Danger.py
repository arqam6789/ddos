...
TOKEN = '7277123875:AAHb8jMdx7Pq-HYqfemeCDx5MX4TpXF76kw'
...
FORWARD_CHANNEL_ID = -1002143131667
CHANNEL_ID = -1002143131667
error_channel_id = -1002143131667
...
def attack_command(message):
    ...
    if message.chat.type != 'private':
        bot.send_message(chat_id, "*The /attack command can only be used in private messages.*", parse_mode='Markdown')
        return
    try:
        user_data = users_collection.find_one({"user_id": user_id})
        if not user_data or user_data['plan'] == 0:
            bot.send_message(chat_id, "*You are not approved to use this bot. \nPlease contact @suicidebombs*", parse_mode='Markdown')
            return
